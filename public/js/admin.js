...
function switchSection(sectionId) {
  ...
  if (sectionId === 'tests') loadTestsManager();
}
...
function openEditChapterModal(chapterId) {
  const chapter = allSubjectChapters.find(c => String(c.id) === String(chapterId));
  if (!chapter) return;
      ...
      <tr>
        ...
        <td>
          <div style="display: flex; gap: 6px; flex-wrap: wrap;">
            <span style="font-size: 11px; padding: 2px 6px; background: #F3F4F6; border-radius: 4px;">📹 ${ch.lectures_count ?? ch.lecture_count ?? 0} Lectures</span>
            <span style="font-size: 11px; padding: 2px 6px; background: #F3F4F6; border-radius: 4px;">📑 ${ch.notes_count ?? 0} Notes</span>
            <span style="font-size: 11px; padding: 2px 6px; background: #F3F4F6; border-radius: 4px;">📝 ${ch.dpp_quizzes_count ?? ch.quizzes_count ?? 0} Quizzes</span>
          </div>
        </td>
        <td>
          <div style="display: flex; gap: 6px; flex-wrap: wrap;">
        ...
        <tr>
          ...
          <td>
            <div style="display: flex; gap: 6px; flex-wrap: wrap; align-items: center;">
              ${scheduleBadge}
              <button class="btn-sm-edit" style="font-size: 10px; font-weight: 700; padding: 3px 6px; border-radius: 6px; cursor: pointer; background: ${(v.is_live === true || v.is_live === 1 || v.is_live === '1') ? '#FEE2E2; color: #DC2626; border: 1px solid #FCA5A5;' : '#F3F4F6; color: #64748B; border: 1px solid #E5E7EB;'};" onclick="toggleVideoLive(${v.id}, ${(v.is_live === true || v.is_live === 1 || v.is_live === '1') ? 0 : 1})" title="Toggle Live Lecture status">
                ${(v.is_live === true || v.is_live === 1 || v.is_live === '1') ? '🔴 LIVE' : '⚪ Live OFF'}
              </button>
            </div>
          </td>
          <td>
...
async function openEditUnifiedLectureModal(lectureId) {
  try {
    ...
    const dlEdit = document.getElementById('unified-lec-download-url');
    if (dlEdit) dlEdit.value = lec.download_url || '';
    ...
    document.getElementById('unified-lec-duration').value = (lec.duration !== undefined && lec.duration !== null) ? lec.duration : '';
    ...
    const isLiveEl = document.getElementById('unified-lec-is-live');
    if (isLiveEl) isLiveEl.checked = (lec.is_live === true || lec.is_live === 1 || lec.is_live === '1');
    const isPubEl = document.getElementById('unified-lec-is-published');
    if (isPubEl) isPubEl.checked = lec.is_published !== 0;

    // 2. Notes PDF (API returns nested on lecture object OR flat fields)
    const notesData = data.notes || lec.notes || (lec.notes_title || lec.notes_pdf_url ? { title: lec.notes_title, external_link: lec.notes_pdf_url, file_size: lec.notes_file_size } : null);
    if (notesData && (notesData.title || notesData.external_link)) {
      document.getElementById('unified-notes-title').value = notesData.title || '';
      document.getElementById('unified-notes-pdf-url').value = notesData.external_link || '';
      document.getElementById('unified-notes-size').value = notesData.file_size || '2.4 MB';
    } else {
      document.getElementById('unified-notes-title').value = '';
      document.getElementById('unified-notes-pdf-url').value = '';
      document.getElementById('unified-notes-size').value = '';
    }

    // 3. DPP PDF
    const dppPdfData = data.dpp_pdf || lec.dpp_pdf || (lec.dpp_pdf_title || lec.dpp_pdf_url ? { title: lec.dpp_pdf_title, external_link: lec.dpp_pdf_url, file_size: lec.dpp_pdf_file_size } : null);
    if (dppPdfData && (dppPdfData.title || dppPdfData.external_link)) {
      document.getElementById('unified-dpp-pdf-title').value = dppPdfData.title || '';
      document.getElementById('unified-dpp-pdf-url').value = dppPdfData.external_link || '';
      document.getElementById('unified-dpp-pdf-size').value = dppPdfData.file_size || '1.5 MB';
    } else {
      document.getElementById('unified-dpp-pdf-title').value = '';
      document.getElementById('unified-dpp-pdf-url').value = '';
      document.getElementById('unified-dpp-pdf-size').value = '';
    }

    // 4. DPP Video
    const dppVidData = data.dpp_video || lec.dpp_video || (lec.dpp_video_title || lec.dpp_video_url ? { title: lec.dpp_video_title, external_link: lec.dpp_video_url, duration: lec.dpp_video_duration } : null);
    if (dppVidData && (dppVidData.title || dppVidData.external_link)) {
      document.getElementById('unified-dpp-vid-title').value = dppVidData.title || '';
      document.getElementById('unified-dpp-vid-url').value = dppVidData.external_link || '';
      document.getElementById('unified-dpp-vid-duration').value = dppVidData.duration || '';
    } else {
      document.getElementById('unified-dpp-vid-title').value = '';
      document.getElementById('unified-dpp-vid-url').value = '';
      document.getElementById('unified-dpp-vid-duration').value = '';
    }

    // 5. DPP Quiz
    const quizData = data.quiz || lec.quiz || (lec.dpp_quiz_title ? { title: lec.dpp_quiz_title, total_questions: lec.dpp_quiz_total_questions, duration_mins: lec.dpp_quiz_duration } : null);
    if (quizData && quizData.title) {
      document.getElementById('unified-quiz-title').value = quizData.title || '';
      document.getElementById('unified-quiz-questions').value = quizData.total_questions || 10;
      document.getElementById('unified-quiz-duration').value = quizData.duration_mins || quizData.duration || 15;
    } else {
    ...
    if (extraList) {
      extraList.innerHTML = '';
      const extras = data.extra_resources || lec.extra_resources || [];
      if (Array.isArray(extras) && extras.length > 0) {
        extras.forEach(r => addUnifiedExtraResourceRow(r));
      }
    }
...
async function saveUnifiedLecture(e) {
  ...
  const payload = {
    ...
    download_url: (document.getElementById('unified-lec-download-url')?.value || '').trim(),
    lecture_duration: document.getElementById('unified-lec-duration').value.trim() || '',
...
function openEditVideoModalItem(videoId) {
  const video = currentChapterData?.lectures?.find(v => String(v.id) === String(videoId)) || currentChapterData?.dpp_videos?.find(v => String(v.id) === String(videoId));
  if (!video) return;
  ...
  document.getElementById('edit-video-duration').value = (video.duration !== undefined && video.duration !== null) ? video.duration : '';
...
function openEditPdfModalItem(pdfId) {
  const pdf = currentChapterData?.notes?.find(p => String(p.id) === String(pdfId)) || currentChapterData?.dpp_pdfs?.find(p => String(p.id) === String(pdfId));
  if (!pdf) return;
...
function openEditSubjectModal(subId) {
  ...
  if (!sub) {
    adminToast('Subject not found. Please refresh the page.', 'error');
    return;
  }
...
async function loadContentManagerLectures(chapterId) {
  ...
  try {
    ...
    tbody.innerHTML = lectures.map((v, idx) => {
      const attachedNotes = data.notes?.find(n => Number(n.lecture_id) === Number(v.id) || (Number(n.chapter_id) === Number(v.chapter_id) && Number(n.display_order) === Number(v.display_order)));
      const attachedDppPdf = data.dpp_pdfs?.find(d => Number(d.lecture_id) === Number(v.id) || (Number(d.chapter_id) === Number(v.chapter_id) && Number(d.display_order) === Number(v.display_order)));
        ...
        <tr>
          ...
          <td>
            ...
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
              ...
              <span style="font-size: 11.5px; color: #64748B;">⏱️ ${v.duration || '—'}</span>
            </div>
          </td>
          <td>
...
function formatAnnouncementMedia(url) {
  if (!url) return '';
  const u = String(url).trim();
  if (!u) return '';
  const low = u.toLowerCase().split('?')[0];
  const isImg = /\.(jpg|jpeg|png|gif|webp|bmp|svg|avif)$/i.test(low) ||
    /unsplash|imgur|ibb\.co|cloudinary|googleusercontent/i.test(u);
  const isPdf = /\.pdf$/i.test(low) || /\.pdf\?/i.test(u);
  if (isImg) {
    return `<img src="${u}" alt="" style="max-width:180px;border-radius:8px;margin-bottom:6px;" onerror="this.style.display='none'" />`;
  }
  if (isPdf) {
    return `<a href="${u}" target="_blank" rel="noopener" style="display:inline-block;margin-bottom:6px;color:#DC2626;font-weight:700;font-size:13px;">📄 Open PDF</a>`;
  }
  return `<a href="${u}" target="_blank" rel="noopener" style="display:inline-block;margin-bottom:6px;color:#7C3AED;font-weight:700;font-size:13px;">🔗 Open Link</a>`;
}
  ...
  try {
    ...
    list.innerHTML = announcements
      ...
      <div style="padding: 16px 20px; border-bottom: 1px solid #E5E7EB; display: flex; align-items: flex-start; justify-content: space-between; gap: 16px;">
        <div>
          ${a.title ? `<p style="font-size: 15px; color: #111827; font-weight: 700; margin-bottom: 4px;">${a.title}</p>` : ''}
          ${formatAnnouncementMedia(a.image_url)}
          <p style="font-size: 14.5px; color: #111827; font-weight: 500; margin-bottom: 6px;">${a.message || a.description || ''}</p>
          <span style="font-size: 12px; color: #6B7280;">${a.created_at ? new Date(a.created_at).toLocaleString() : ''}</span>
        </div>
...
async function createAnnouncement(e) {
  ...
  const imageEl = document.getElementById('ann-image-input');
  const messageEl = document.getElementById('ann-message-input');
  const title = titleEl ? titleEl.value.trim() : '';
  const image_url = imageEl ? imageEl.value.trim() : '';
  const message = messageEl ? messageEl.value.trim() : '';

  if (!title && !message && !image_url) {
    adminToast('Add at least a title, message, or photo link', 'error');
    return;
  }
  ...
  try {
    const res = await adminFetch('/api/admin/announcements', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ batch_id: batchId, title, message, image_url, description: message })
    });
    if (res.ok) {
      adminToast('Announcement posted!');
      ...
      const data = await res.json().catch(() => ({}));
      adminToast(data.error || 'Failed to post', 'error');
    }
  } catch (err) {
...
async function loadTeachersTable() {
  try {
    ...
    tbody.innerHTML = allTeachers.map((t, idx) => {
      const batchObj = allBatches.find(b => String(b.id) === String(t.batch_id));
...
function openEditTeacherModal(teacherId) {
  const t = allTeachers.find(item => String(item.id) === String(teacherId));
  if (!t) {
    adminToast('Teacher not found. Please refresh the page.', 'error');
    return;
  }
...
async function saveTeacher(event) {
  ...
  const payload = {
    batch_id: batchVal ? String(batchVal) : null,
...
// --- BATCH TESTS MANAGER ---
async function loadTestsManager() {
  const batchSelect = document.getElementById('test-batch-select');
  if (!batchSelect) return;
  if (allBatches.length === 0) {
    const res = await adminFetch('/api/admin/batches');
    const data = await res.json();
  ...
  batchSelect.innerHTML = allBatches.map(b => `<option value="${b.id}">${b.title}</option>`).join('');
  if (allBatches.length > 0) loadTestsForBatch(batchSelect.value || allBatches[0].id);
}

async function loadTestsForBatch(batchId) {
  const list = document.getElementById('tests-list');
  if (!list) return;
  try {
    const res = await adminFetch(`/api/admin/tests?batch_id=${encodeURIComponent(batchId)}`);
    const data = await res.json();
    const tests = data.tests || [];
    if (tests.length === 0) {
      list.innerHTML = '<div style="padding:24px;text-align:center;color:#6B7280;">No tests for this batch yet.</div>';
      return;
    }
    list.innerHTML = tests.map(t => `
      <div style="padding:16px 20px;border-bottom:1px solid #E5E7EB;display:flex;justify-content:space-between;gap:16px;align-items:flex-start;">
        <div>
          <p style="font-weight:700;color:#111827;">${t.title}</p>
          ${t.scheduled_date ? `<span style="font-size:12px;color:#7C3AED;">📅 ${t.scheduled_date}</span>` : ''}
          ${t.external_link ? `<div style="font-size:13px;margin-top:4px;"><a href="${t.external_link}" target="_blank" rel="noopener">Open link</a></div>` : '<div style="font-size:12px;color:#6B7280;">Scheduled (no link yet)</div>'}
          ${t.description ? `<p style="font-size:13px;color:#374151;margin-top:4px;">${t.description}</p>` : ''}
        </div>
        <button class="btn-sm-delete" onclick="deleteTestItem(${t.id}, '${batchId}')">Delete</button>
      </div>
    `).join('');
  } catch (err) {
    list.innerHTML = '<div style="padding:24px;color:#EF4444;">Failed to load tests</div>';
  }
}

async function createTestItem(e) {
  e.preventDefault();
  ...
  const link = document.getElementById('test-link-input').value.trim();
  const date = document.getElementById('test-date-input').value;
  const desc = document.getElementById('test-desc-input').value.trim();
  if (!title) {
    adminToast('Test title required', 'error');
    return;
  }
  try {
    const res = await adminFetch('/api/admin/tests', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ batch_id: batchId, title, external_link: link, scheduled_date: date, description: desc })
    });
    if (res.ok) {
      adminToast('Test saved!');
      document.getElementById('test-title-input').value = '';
      document.getElementById('test-link-input').value = '';
      document.getElementById('test-date-input').value = '';
      document.getElementById('test-desc-input').value = '';
      loadTestsForBatch(batchId);
    } else {
      const data = await res.json().catch(() => ({}));
      adminToast(data.error || 'Failed', 'error');
    }
  } catch (err) {
    adminToast('Error saving test', 'error');
  }
}

async function deleteTestItem(id, batchId) {
  if (!confirm('Delete this test?')) return;
  try {
    const res = await adminFetch(`/api/admin/tests/${id}`, { method: 'DELETE' });
    if (res.ok) {
      adminToast('Test deleted');
      loadTestsForBatch(batchId);
    } else {
      adminToast('Failed to delete', 'error');
    }
  } catch (err) {
