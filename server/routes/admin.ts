...
// Alias used by admin frontend saveUnifiedLecture (edit mode)
router.put('/lectures/:id/unified', (req: Request, res: Response) => {
  const updated = db.updateUnifiedLecture(Number(req.params.id), req.body);
  if (!updated) {
    res.status(404).json({ error: 'Lecture not found' });
    return;
...
router.post('/announcements', (req: Request, res: Response) => {
  const { batch_id, message, title, image_url, description } = req.body;
  if (!batch_id) {
    res.status(400).json({ error: 'Batch ID is required' });
    return;
  }
  // All content fields optional — at least one of title/message/image preferred
  if (!title && !message && !image_url && !description) {
    res.status(400).json({ error: 'Provide at least a title, message, or image link' });
    return;
  }
  try {
    ...
    res.status(400).json({ error: e.message || 'Failed to create announcement' });
  }
});
...
router.get('/tests', (req: Request, res: Response) => {
  ...
  const tests = db.getTestsByBatch(batchId);
  res.json({ tests });
});

router.post('/tests', (req: Request, res: Response) => {
  const { batch_id, title, external_link, link, description, scheduled_date } = req.body;
  if (!batch_id) {
    res.status(400).json({ error: 'Batch ID is required' });
    return;
  }
...
router.delete('/tests/:id', async (req: Request, res: Response) => {
  const success = await db.deleteTest(Number(req.params.id));
  if (!success) {
