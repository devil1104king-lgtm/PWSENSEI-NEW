...
async function syncFromDatabase(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      // Normalize flags — PG/Cockroach may return string "0"/"1" which is truthy in JS
      lectures = lecturesRes.rows.map((row: any) => ({
        ...row,
        is_live: Number(row.is_live) === 1 ? 1 : 0,
        is_today: Number(row.is_today) === 1 ? 1 : 0,
        is_published: row.is_published === undefined || row.is_published === null ? 1 : (Number(row.is_published) === 0 ? 0 : 1),
        chapter_id: Number(row.chapter_id),
        id: Number(row.id),
        lecture_number: row.lecture_number !== undefined && row.lecture_number !== null ? Number(row.lecture_number) : row.lecture_number,
        display_order: row.display_order !== undefined && row.display_order !== null ? Number(row.display_order) : row.display_order
      }));
    } catch (e) {
    ...
    try {
      ...
      notes = notesRes.rows.map((row: any) => ({
        ...row,
        id: Number(row.id),
        chapter_id: Number(row.chapter_id),
        lecture_id: row.lecture_id !== undefined && row.lecture_id !== null ? Number(row.lecture_id) : null,
        is_published: row.is_published === undefined || row.is_published === null ? 1 : (Number(row.is_published) === 0 ? 0 : 1),
        display_order: row.display_order !== undefined && row.display_order !== null ? Number(row.display_order) : row.display_order
      }));
    } catch (e) {
    ...
    try {
      ...
      extraResources = extrasRes.rows.map((row: any) => ({
        ...row,
        id: Number(row.id),
        lecture_id: row.lecture_id !== undefined && row.lecture_id !== null ? Number(row.lecture_id) : null,
        display_order: row.display_order !== undefined && row.display_order !== null ? Number(row.display_order) : row.display_order
...
export async function initDatabase(): Promise<void> {
  ...
  try {
    ...
    const migrations = [
      ...
      'ALTER TABLE lectures ADD COLUMN IF NOT EXISTS download_url TEXT',
    ...
    // First-install only: seed when BOTH settings and batches tables are empty.
    // Deleting all batches must NOT re-inject dummy batches or overwrite site settings (bot links etc.).
    let settingsCount = 0;
    try {
      const sCnt = await pgPool.query('SELECT COUNT(*) as cnt FROM settings');
      settingsCount = parseInt(sCnt.rows[0]?.cnt || '0', 10);
    } catch (_) {
      settingsCount = 0;
    }
    ...
    if (settingsCount === 0 && count === 0) {
      console.log('🌱 First install — seeding initial data into PostgreSQL/CockroachDB...');
      const seed = loadDatabase();
      
      for (const [k, v] of Object.entries(seed.settings)) {
        // Never overwrite existing admin settings
...
export const db = {
  ...
  getBatchById(id: string): any {
    ...
    const tests = (db.tests || [])
      .filter(x => String(x.batch_id) === String(id) && Number(x.is_published) !== 0)
  ...
  getSubjectById(id: number): any {
    ...
    const chapters = db.chapters
      ...
      .map(c => {
        ...
        const lectureCount = lecs.filter(l => l.video_type !== 'dpp_video').length;
        const notesCount = nts.filter(n => n.type === 'note').length;
        const dppPdfCount = nts.filter(n => n.type === 'dpp_pdf').length;
        const dppVidCount = lecs.filter(l => l.video_type === 'dpp_video').length;
        const quizCount = qzs.length;
        return {
          ...c,
          lecture_count: lectureCount,
          lectures_count: lectureCount,
          notes_count: notesCount,
          dpp_count: dppPdfCount + dppVidCount,
          dpp_pdfs_count: dppPdfCount,
          dpp_videos_count: dppVidCount,
          dpp_quizzes_count: quizCount,
          quiz_count: quizCount,
          total_content_count: lectureCount + notesCount + dppPdfCount + dppVidCount + quizCount
        };
      });
  ...
  // --- CHAPTERS ---
  getChaptersBySubject(subjectId: number): any[] {
    const db = loadDatabase();
    return db.chapters
      ...
      .map(c => {
        ...
        const lecture_count = db.lectures.filter(l => Number(l.chapter_id) === cid && l.video_type !== 'dpp_video').length;
        const notes_count = db.notes.filter(n => Number(n.chapter_id) === cid && n.type === 'note').length;
        const dpp_quizzes_count = db.quizzes.filter(q => Number(q.chapter_id) === cid).length;
        return {
          ...c,
          lecture_count,
          ...
          notes_count,
          dpp_quizzes_count,
          quizzes_count: dpp_quizzes_count
        };
      });
  },
  ...
  getChapterById(id: number): any {
    ...
    const subject = db.subjects.find(s => Number(s.id) === Number(chapter.subject_id));
    ...
    // is_published null/undefined = treat as published (don't hide notes)
    const isPub = (v: any) => v === undefined || v === null || Number(v) !== 0;
    const rawNotes = db.notes.filter(n => Number(n.chapter_id) === Number(chapter.id) && isPub(n.is_published) && n.type === 'note');
    const rawDppPdfs = db.notes.filter(n => Number(n.chapter_id) === Number(chapter.id) && isPub(n.is_published) && n.type === 'dpp_pdf');
    const rawQuizzes = db.quizzes.filter(q => Number(q.chapter_id) === Number(chapter.id) && isPub(q.is_published));

    // Attach nested notes, dpp_pdf, dpp_video, dpp_quiz, and extra_resources to each lecture
    // Always Number() — CockroachDB may return string ids (strict === fails otherwise)
    const populatedLectures = rawLectures.map(lec => {
      const lecId = Number(lec.id);
      const lecNum = Number(lec.lecture_number);
      const chIdNum = Number(chapter.id);

      let lecNote = db.notes.find(n => Number(n.lecture_id) === lecId && n.type === 'note');
      if (!lecNote && !isNaN(lecNum)) {
        lecNote = db.notes.find(n =>
          Number(n.chapter_id) === chIdNum &&
          n.type === 'note' &&
          Number(n.display_order) === lecNum
        );
      }

      let lecDppPdf = db.notes.find(n => Number(n.lecture_id) === lecId && n.type === 'dpp_pdf');
      if (!lecDppPdf && !isNaN(lecNum)) {
        lecDppPdf = db.notes.find(n =>
          Number(n.chapter_id) === chIdNum &&
          n.type === 'dpp_pdf' &&
          Number(n.display_order) === lecNum
        );
      ...
      const lecDppVid = db.lectures.find(l =>
        Number(l.chapter_id) === chIdNum &&
        l.video_type === 'dpp_video' &&
        Number(l.lecture_number) === lecNum
      );
      const lecQuiz = db.quizzes.find(q => Number(q.lecture_id) === lecId);
      const extras = db.extra_resources.filter(e => Number(e.lecture_id) === lecId);
  ...
  getAdminChapterContent(chapterId: number): any {
    ...
    const lecs = db.lectures.filter(l => Number(l.chapter_id) === Number(chId) && l.video_type !== 'dpp_video');
    const dppVideos = db.lectures.filter(l => Number(l.chapter_id) === Number(chId) && l.video_type === 'dpp_video');
    const notes = db.notes.filter(n => Number(n.chapter_id) === Number(chId) && n.type === 'note');
    const dppPdfs = db.notes.filter(n => Number(n.chapter_id) === Number(chId) && n.type === 'dpp_pdf');
    ...
    const populatedLecs = lecs.map(l => {
      ...
      const lecNum = Number(l.lecture_number);
      const extras = db.extra_resources.filter(e => Number(e.lecture_id) === lid);
      let note = db.notes.find(n => Number(n.lecture_id) === lid && n.type === 'note');
      if (!note && !isNaN(lecNum)) {
        note = db.notes.find(n => Number(n.chapter_id) === chId && n.type === 'note' && Number(n.display_order) === lecNum);
      }
      let dppPdf = db.notes.find(n => Number(n.lecture_id) === lid && n.type === 'dpp_pdf');
      if (!dppPdf && !isNaN(lecNum)) {
        dppPdf = db.notes.find(n => Number(n.chapter_id) === chId && n.type === 'dpp_pdf' && Number(n.display_order) === lecNum);
      }
      const dppVid = db.lectures.find(v =>
        Number(v.chapter_id) === chId &&
        v.video_type === 'dpp_video' &&
        Number(v.lecture_number) === lecNum
      );
      const quiz = db.quizzes.find(q => Number(q.lecture_id) === lid);
  ...
  getLectureById(lectureId: number): any {
    ...
    const lecId = Number(lec.id);
    const extras = db.extra_resources.filter(e => Number(e.lecture_id) === lecId);
    const note = db.notes.find(n => Number(n.lecture_id) === lecId && n.type === 'note');
    const dppPdf = db.notes.find(n => Number(n.lecture_id) === lecId && n.type === 'dpp_pdf');
    const dppVid = db.lectures.find(v =>
      Number(v.chapter_id) === Number(lec.chapter_id) &&
      v.video_type === 'dpp_video' &&
      Number(v.lecture_number) === Number(lec.lecture_number)
    );
    const quiz = db.quizzes.find(q => Number(q.lecture_id) === lecId);

    const notesObj = note ? {
      id: note.id,
      title: note.title || '',
      external_link: note.external_link || '',
      file_size: note.file_size || '2.4 MB',
      type: note.type || 'note'
    } : null;
    const dppPdfObj = dppPdf ? {
      id: dppPdf.id,
      title: dppPdf.title || '',
      external_link: dppPdf.external_link || '',
      file_size: dppPdf.file_size || '1.5 MB',
      type: dppPdf.type || 'dpp_pdf'
    } : null;
    const dppVidObj = dppVid ? {
      id: dppVid.id,
      title: dppVid.title || '',
      external_link: dppVid.external_link || '',
      duration: dppVid.duration || ''
    } : null;
    const quizObj = quiz ? {
      id: quiz.id,
      title: quiz.title || '',
      total_questions: quiz.total_questions || 10,
      duration_mins: quiz.duration || 15,
    ...
    return {
      ...
      dpp_video_duration: dppVid?.duration || '',
  ...
  createUnifiedLecture(chapterId: number, payload: any): any {
    ...
    const newLecture: Lecture = {
      ...
      download_url: (payload.download_url !== undefined && payload.download_url !== null) ? String(payload.download_url).trim() : '',
      duration: (payload.lecture_duration !== undefined && payload.lecture_duration !== null) ? String(payload.lecture_duration).trim() : '',
    ...
    if (payload.dpp_video_title && payload.dpp_video_url) {
      db.lectures.push({
        ...
        duration: (payload.dpp_video_duration !== undefined && payload.dpp_video_duration !== null) ? String(payload.dpp_video_duration).trim() : '',
        teacher_name: payload.teacher_name || '',
        lecture_date: payload.lecture_date || new Date().toISOString().split('T')[0],
        is_live: 0,
        is_today: 0,
      ...
      (async () => {
        try {
          ...
          if (lec) {
            ...
            // Safe: only sets new optional column; never deletes existing lecture data
            try {
              await pgPool.query(`UPDATE lectures SET download_url = $1 WHERE id = $2`, [lec.download_url || '', lec.id]);
            } catch (_) { /* column may not exist yet on very old DB before migration */ }
          }
  ...
  updateUnifiedLecture(lectureId: number, payload: any): any {
    ...
    db.lectures[lecIdx] = {
      ...
      download_url: payload.download_url !== undefined ? String(payload.download_url).trim() : (existingLec.download_url || ''),
      duration: (payload.lecture_duration !== undefined && payload.lecture_duration !== null) ? String(payload.lecture_duration).trim() : (existingLec.duration || ''),
    ...
    // Update or Insert Note — NEVER auto-delete on empty form (only update/create when data given)
    let existingNoteIdx = db.notes.findIndex(n => Number(n.lecture_id) === lecId && n.type === 'note');
    if (existingNoteIdx === -1 && !isNaN(lecNumber)) {
      existingNoteIdx = db.notes.findIndex(n =>
        Number(n.chapter_id) === chId && n.type === 'note' && Number(n.display_order) === lecNumber
      );
    }
    if ((payload.notes_title && String(payload.notes_title).trim()) || (payload.notes_pdf_url && String(payload.notes_pdf_url).trim())) {
    ...
    // Empty form fields = leave existing notes untouched (do NOT delete)

    // Update or Insert DPP PDF — never auto-delete on empty form
    let existingDppPdfIdx = db.notes.findIndex(n => Number(n.lecture_id) === lecId && n.type === 'dpp_pdf');
    if (existingDppPdfIdx === -1 && !isNaN(lecNumber)) {
      existingDppPdfIdx = db.notes.findIndex(n =>
        Number(n.chapter_id) === chId && n.type === 'dpp_pdf' && Number(n.display_order) === lecNumber
      );
    }
    if ((payload.dpp_pdf_title && String(payload.dpp_pdf_title).trim()) || (payload.dpp_pdf_url && String(payload.dpp_pdf_url).trim())) {
      const dppTitle = (payload.dpp_pdf_title && String(payload.dpp_pdf_title).trim()) || 'DPP PDF';
      const dppUrl = (payload.dpp_pdf_url && String(payload.dpp_pdf_url).trim()) || '';
      if (existingDppPdfIdx !== -1) {
        db.notes[existingDppPdfIdx] = {
          ...
          chapter_id: chId,
          title: dppTitle,
          external_link: dppUrl || db.notes[existingDppPdfIdx].external_link,
          file_size: payload.dpp_pdf_file_size || db.notes[existingDppPdfIdx].file_size || '1.5 MB',
          display_order: lecNumber
        };
      } else {
        db.notes.push({
    ...
    // Update or Insert DPP Video — never auto-delete on empty form
    let existingDppVidIdx = db.lectures.findIndex(v =>
      Number(v.chapter_id) === chId && v.video_type === 'dpp_video' && Number(v.lecture_number) === lecNumber
    );
    if ((payload.dpp_video_title && String(payload.dpp_video_title).trim()) || (payload.dpp_video_url && String(payload.dpp_video_url).trim())) {
      if (existingDppVidIdx !== -1) {
        db.lectures[existingDppVidIdx] = {
          ...db.lectures[existingDppVidIdx],
          title: (payload.dpp_video_title && String(payload.dpp_video_title).trim()) || db.lectures[existingDppVidIdx].title,
          external_link: (payload.dpp_video_url && String(payload.dpp_video_url).trim()) || db.lectures[existingDppVidIdx].external_link,
          duration: (payload.dpp_video_duration !== undefined && payload.dpp_video_duration !== null)
            ? String(payload.dpp_video_duration).trim()
            : (db.lectures[existingDppVidIdx].duration || '')
        };
      } else if (payload.dpp_video_title && payload.dpp_video_url) {
        db.lectures.push({
          id: db.nextId.lecture++,
          chapter_id: chId,
          title: payload.dpp_video_title,
    ...
    // Empty quiz title = leave existing quiz untouched (do NOT auto-delete)

    // Update Extra Resources — only replace when form actually sends items; empty = keep existing
    if (Array.isArray(payload.extra_resources) && payload.extra_resources.length > 0) {
      db.extra_resources = db.extra_resources.filter(e => Number(e.lecture_id) !== lecId);
      payload.extra_resources.forEach((r: any, idx: number) => {
        if (r.title && r.url) {
          db.extra_resources.push({
            id: db.nextId.extra_resource++,
    ...
    if (pgPool) {
      (async () => {
        try {
          ...
          for (const n of mem.notes.filter(n => Number(n.lecture_id) === Number(updatedLec.id))) {
            await pgPool.query(
              `INSERT INTO notes (id, chapter_id, lecture_id, title, external_link, file_size, type, display_order, is_published)
               VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
               ON CONFLICT (id) DO UPDATE SET
          ...
          for (const q of mem.quizzes.filter(q => Number(q.lecture_id) === Number(updatedLec.id))) {
            await pgPool.query(
              `INSERT INTO quizzes (id, chapter_id, lecture_id, title, total_questions, duration, external_link, is_published)
               VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
               ON CONFLICT (id) DO UPDATE SET
  ...
  createVideo(data: Partial<Lecture>): Lecture {
    ...
    const newLec: Lecture = {
      ...
      duration: (data.duration !== undefined && data.duration !== null) ? String(data.duration).trim() : '',
  ...
  createAnnouncement(batchIdOrData: string | any, maybeMessage?: string): Announcement {
    const db = loadDatabase();
    // Support: createAnnouncement(batchId, message) OR createAnnouncement({ batch_id, title, message, image_url, description })
    let batchId = '';
    let title = '';
    let message = '';
    let image_url = '';
    ...
    if (typeof batchIdOrData === 'object' && batchIdOrData !== null) {
      batchId = String(batchIdOrData.batch_id || '');
      title = String(batchIdOrData.title || '').trim();
      message = String(batchIdOrData.message || batchIdOrData.description || '').trim();
      image_url = String(batchIdOrData.image_url || batchIdOrData.photo_url || '').trim();
      description = String(batchIdOrData.description || '').trim();
      if (!message && description) message = description;
    } else {
      batchId = String(batchIdOrData || '');
      message = String(maybeMessage || '').trim();
    }
    // At least one of title / message / image required
    if (!batchId) {
      throw new Error('Batch ID is required');
    }
    if (!title && !message && !image_url) {
      // allow empty-ish soft: store placeholder title
      title = 'Announcement';
    }
    ...
    if (pgPool) {
      (async () => {
        try {
          await pgPool.query(
            `INSERT INTO announcements (id, batch_id, message, title, image_url, description)
             VALUES ($1, $2, $3, $4, $5, $6)
            ...
            [newAnn.id, newAnn.batch_id, newAnn.message, newAnn.title || null, newAnn.image_url || null, newAnn.description || null]
          );
        } catch (err) {
          ...
          try {
            ...
            console.error('Error creating announcement in PostgreSQL/CockroachDB:', err2);
          }
        }
      })();
    }
  ...
  getTodayLectures(batchId?: string): Lecture[] {
    const db = loadDatabase();
    // Today's date in IST (Asia/Kolkata) — reliable, no UTC shift
    let todayStr = '';
    try {
      todayStr = new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Kolkata' }).format(new Date());
    } catch (e) {
      const nowIst = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
    ...
    // TODAY = only lectures whose lecture_date is exactly today (IST)
    // Past dates never show as Today. is_today flag is ignored (stale).
  ...
  getUpcomingLectures(batchId?: string): Lecture[] {
    ...
    try {
      ...
      const nowIst = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
      todayStr = `${nowIst.getFullYear()}-${String(nowIst.getMonth()+1).padStart(2,'0')}-${String(nowIst.getDate()).padStart(2,'0')}`;
    }
  ...
  getTestsByBatch(batchId: string): BatchTest[] {
    const db = loadDatabase();
    if (!db.tests) db.tests = [];
    return (db.tests || [])
      .filter(x => String(x.batch_id) === String(batchId) && Number(x.is_published) !== 0)
      .sort((a, b) => (Number(a.display_order) || 0) - (Number(b.display_order) || 0));
  },

  createTest(data: Partial<BatchTest>): BatchTest {
    const db = loadDatabase();
    if (!db.tests) db.tests = [];
    if (!db.nextId.test) db.nextId.test = 1;
    const row: BatchTest = {
      id: db.nextId.test++,
      batch_id: String(data.batch_id || ''),
      title: String(data.title || 'Test').trim() || 'Test',
      external_link: String(data.external_link || data.link || '').trim(),
      description: String(data.description || '').trim(),
      scheduled_date: String(data.scheduled_date || '').trim(),
      is_published: data.is_published !== undefined ? Number(data.is_published) : 1,
      display_order: Number(data.display_order) || 0,
      created_at: new Date().toISOString()
    };
    ...
    if (pgPool) {
      (async () => {
        try {
          await pgPool.query(
            `INSERT INTO tests (id, batch_id, title, external_link, description, scheduled_date, is_published, display_order, created_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) ON CONFLICT (id) DO NOTHING`,
            [row.id, row.batch_id, row.title, row.external_link, row.description, row.scheduled_date, row.is_published, row.display_order, row.created_at]
          );
        } catch (err) {
  ...
  async deleteTest(id: number): Promise<boolean> {
    ...
    if (!db.tests) db.tests = [];
    const numId = Number(id);
    const initLen = db.tests.length;
    db.tests = db.tests.filter(x => Number(x.id) !== numId);
    saveDatabase();
    if (pgPool) {
      try {
        await pgPool.query('DELETE FROM tests WHERE id = $1', [numId]);
      } catch (err) {
        console.error('Error deleting test:', err);
      }
    }
    return db.tests.length !== initLen;
  },
