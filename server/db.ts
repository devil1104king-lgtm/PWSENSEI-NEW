...
export const db = {
  ...
  getTodayLectures(batchId?: string): Lecture[] {
    const db = loadDatabase();
    let result = db.lectures.filter(l => Number(l.is_published) !== 0 && (l.is_today === 1 || l.is_live === 1));

    if (batchId) {
      const subIds = db.subjects.filter(s => String(s.batch_id) === String(batchId)).map(s => s.id);
      const chIds = db.chapters.filter(c => subIds.map(Number).includes(Number(c.subject_id))).map(c => c.id);
      result = result.filter(l => chIds.map(Number).includes(Number(l.chapter_id)));
    }
  ...
  getUpcomingLectures(batchId?: string): Lecture[] {
    const db = loadDatabase();
    let result = db.lectures.filter(l => Number(l.is_published) !== 0 && l.is_live !== 1 && l.is_today !== 1);
