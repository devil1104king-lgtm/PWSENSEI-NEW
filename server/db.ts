...
export const db = {
  ...
  getAdminChapterContent(chapterId: number): any {
    ...
    const populatedLecs = lecs.map(l => {
      const extras = db.extra_resources.filter(e => Number(e.lecture_id) === Number(l.id));
      const note = db.notes.find(n => Number(n.lecture_id) === Number(l.id) && n.type === 'note');
      const dppPdf = db.notes.find(n => Number(n.lecture_id) === Number(l.id) && n.type === 'dpp_pdf');
      const dppVid = db.lectures.find(v => v.chapter_id === chId && v.video_type === 'dpp_video' && v.lecture_number === l.lecture_number);
      const quiz = db.quizzes.find(q => Number(q.lecture_id) === Number(l.id));
